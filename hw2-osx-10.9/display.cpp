...
void display() {
  ...
  // Either use the built-in lookAt function or the lookAt implemented by the
  // user.
  if (useGlu) {
  ...
  if (numused) {
    ...
    // glUniform4fv() and similar functions will be useful. See FAQ for help
    // with these functions.
    // The lightransf[] array in variables.h and transformvec() might also be
    // useful here.
  ...
  for (int i = 0; i < numobjects; i++) {
    ...
    glUniform4fv(specularcol, 1, obj->emission);
    glUniform4fv(emissioncol, 1, obj->ambient);
